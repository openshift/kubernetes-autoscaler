...
func testNodeGroupIncreaseMachineSet(t *testing.T, testnum int, tc testNodeGroupResizeTestCase) {
	...
	if !tc.errors {
		...
		ms, err := ng.clusterapiClient.MachineSets(ng.Namespace()).Get(ng.Name(), v1.GetOptions{})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func testNodeGroupIncreaseMachineDeployment(t *testing.T, testnum int, tc testNodeGroupResizeTestCase) {
	...
	if !tc.errors {
		// A nodegroup is immutable; get a fresh copy.
		md, err := ng.clusterapiClient.MachineDeployments(ng.Namespace()).Get(ng.Name(), v1.GetOptions{})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestNodeGroupMachineSetDeleteNodes(t *testing.T) {
	...
	machines := make([]*v1alpha1.Machine, 10)
	...
	for i := 5; i < len(machines); i++ {
		...
		machine, err := controller.clusterClientset.ClusterV1alpha1().Machines("test-namespace").Get(key, v1.GetOptions{})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	machineSet, err = controller.clusterClientset.ClusterV1alpha1().MachineSets(machineSet.Namespace).Get(machineSet.Name, v1.GetOptions{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
