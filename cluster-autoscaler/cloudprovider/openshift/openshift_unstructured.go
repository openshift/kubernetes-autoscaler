...
func (r *unstructuredScalableResource) ProviderIDs() ([]string, error) {
	providerIds, err := r.controller.findScalableResourceProviderIDs(r.unstructured)
	if err != nil {
		return nil, err
	}
...
func (r *unstructuredScalableResource) Labels() map[string]string {
	...
	if labels, found, err := unstructured.NestedStringMap(r.unstructured.Object, "spec", "template", "spec", "metadata", "labels"); found && err == nil {
...
func unstructuredToTaint(unstructuredTaintInterface interface{}) *corev1.Taint {
	unstructuredTaint, ok := unstructuredTaintInterface.(map[string]interface{})
	if unstructuredTaint == nil || !ok {
		return nil
	}

	taint := &corev1.Taint{}
	key, ok := unstructuredTaint["key"].(string)
	if !ok {
		return nil
	}
	taint.Key = key
	...
	if unstructuredTaint["value"] != nil {
		val, ok := unstructuredTaint["value"].(string)
		if ok {
			taint.Value = val
		}
	}

	effect, ok := unstructuredTaint["effect"].(string)
	if !ok {
		return nil
	}
