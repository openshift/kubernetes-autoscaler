...
const (
	// API groups for Machine API resources
	MAPIGroup = "machine.openshift.io"
	// API version for Machine API resources
	MAPIVersion = "v1beta1"
	...
	nodeGroupAutoscalingOptionsKeyPrefix = "machine.openshift.io/autoscaling-options-"
	// machineDeleteAnnotationKey is the annotation used by cluster-api to indicate
	// that a machine should be deleted.
	...
	clusterNameAnnotationKey = "machine.openshift.io/cluster-name"
	// clusterNamespaceAnnotationKey is the annotation used by cluster-api for annotating nodes
	// with their cluster namespace.
	clusterNamespaceAnnotationKey = "machine.openshift.io/cluster-namespace"
	...
	// which will allow the usage of either prefix, while preferring the upstream prefix "capacity.cluster-autoscaler.kuberenetes.io".
...
func filterClusterAPIAuthoritativeResources(mapiGroups []cloudprovider.NodeGroup, capiGroups []cloudprovider.NodeGroup) []cloudprovider.NodeGroup {
	...
	for _, ng := range capiGroups {
		...
		if len(idparts) == 3 {
			...
			if idparts[0] != machineSetKind || idparts[1] != clusterAPINamespace {
				klog.V(5).Infof("skipping filtering for resource %q", ng.Id())
				continue
			}
