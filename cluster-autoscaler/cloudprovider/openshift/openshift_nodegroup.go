...
func (ng *nodegroup) DeleteNodes(nodes []*corev1.Node) error {
	...
	for _, node := range nodes {
		...
		if err != nil {
			// we don't return the error here because the node group may have been deleted.
			// if the node group has been deleted, or the node is in the process of being
			// deleted we want to let the process continue. returning an error here will
			// cause the core autoscaler to think that the delete failed.
			return nil
		}
