...
// setNestedField is a testing helper function to wrap the error detection and fatal failure behavior.
func setNestedField(t *testing.T, obj map[string]interface{}, value interface{}, fields ...string) {
...
func NewTestMachineController(t testing.TB) *testMachineController {
	...
	dynamicClientset := fakedynamic.NewSimpleDynamicClientWithCustomListKinds(
		...
		map[schema.GroupVersionResource]string{
			{Group: MAPIGroup, Version: MAPIVersion, Resource: resourceNameMachine}:    "kindList",
			{Group: MAPIGroup, Version: MAPIVersion, Resource: resourceNameMachineSet}: "kindList",
		},
	)
	discoveryClient := &fakediscovery.FakeDiscovery{
		Fake: &clientgotesting.Fake{
			Resources: []*metav1.APIResourceList{
				{
					GroupVersion: path.Join(MAPIGroup, MAPIVersion),
	...
	scaleReactor := func(action clientgotesting.Action) (bool, runtime.Object, error) {
		...
		if resource != resourceNameMachineSet {
			// Do not attempt to react to resources that are not MachineSet
